...
const PANEL_COUNT = 3;
const PANEL_BASE_DELAY = 0.9; // seconds the cover holds before lifting
const PANEL_STAGGER = 0.08;
const INTRO_UNMOUNT_MS = 1700;
...
export const LandingScreen: React.FC<LandingScreenProps> = ({
  ...
  useEffect(() => {
    const t = setTimeout(() => setIntroDone(true), INTRO_UNMOUNT_MS);
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors relative overflow-hidden">
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          .ledgr-intro-panel, .ledgr-intro-logo, .ledgr-content-in {
        ...
        <div className="fixed inset-0 z-50 flex" aria-hidden="true">
          {Array.from({ length: PANEL_COUNT }).map((_, i) => (
            <div
              ...
              style={{ animationDelay: `${PANEL_BASE_DELAY + i * PANEL_STAGGER}s` }}
            >
              {i === Math.floor(PANEL_COUNT / 2) && (
      ...
      <main className="max-w-7xl mx-auto px-6 pt-6 pb-24 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 border-t border-slate-200 dark:border-slate-800 pt-10">
          ...
          <div className="md:col-span-8">
            ...
            {/* CTA bar, replacing the services tab */}
            <div
          ...
          {/* Right: condensed live stats card, replacing the video panel */}
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800">
                {stats.map((s) => (
                  <div key={s.label} className="px-4 py-3">
                    ...
                    <p className="text-lg font-bold text-slate-900 dark:text-white mt-0.5">{s.value}</p>
